...
const Proyects = () => {
	...
	const FilterProyect = () => {
		if (ServiciosFilter === 'todo') {
			...
			setProyectosFiltrados(dataProyects.filter((data) => data.servicios?.icon?.includes(ServiciosFilter)))
		}
	}
	...
	return (
		<Container bgColor='bg-[#F9FAFB]' className='pt-10 pb-20'>
			...
			<div className='grid grid-cols-1 gap-5 mt-10  md:grid-cols-2 lg:grid-cols-3 justify-items-center'>
			...
			<div className='grid grid-cols-1 gap-5 mt-5  md:grid-cols-2 lg:grid-cols-3 justify-items-center'>
				{show &&
					ProyectosFiltrado.map((obj, index) => {
						if (index > 5) {
							return <ProyectItem key={index} data={obj} />
