    ...
    <div className="py-10 md:py-16 bg-gray-50 " >
      <div className="container mx-auto px-6 md:px-12 xl:px-32 ">
        <div className="mb-10 text-center">
          ...
          {data.subtittle.split('//').map((obj, k) => <p key={k} className="text-garden-option3 lg:w-8/12 lg:mx-auto font-garden_regular text-sm md:text-xl ">{obj || ''}</p>)}
        </div>
          ...
          {data.items.map((obj, i) =>
            <div className="space-y-4 text-center" key={i}>
              <div className="flex flex-col gap-2">
                ...
                <span className="block text-[14px] md:text-base text-garden-option3 font-garden_regular ">{obj.subtittle || ''}</span>
              </div>
            </div>
          )}
        </div>
