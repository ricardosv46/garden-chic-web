...
const InputFloat = ({ label, value, name, onChange, className = 'w-full', type = 'text', htmlFor, ref, rightElement, error, ...props }: IProps) => {
	...
	return (
		<div className='relative w-full'>
			<input
				...
				value={value}
				onChange={onChange}
				className={`${className} block px-2.5  pb-2 pt-5 text-sm text-garden-option3 font-semibold border border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-garden-option1 peer rounded-md`}
				placeholder=''
				ref={ref}
			/>
			<label
