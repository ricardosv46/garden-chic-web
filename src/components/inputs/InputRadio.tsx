...
const InputRadio = ({ label, name, stock, id, value, onChange, defaultChecked = false }: Radio) => {
	return (
		<div className='flex items-center justify-between py-1 relative'>
			<div className='flex items-center gap-x-3 w-4/5'>
				<input
					...
					id={id}
					name={name}
					className='w-5 h-5 rounded checked:bg-garden-option1 peer'
					value={value}
					onChange={onChange}
			...
			<Show condition={typeof stock !== 'undefined'}>
				<div className='rounded-full text-[12px] flex justify-center items-center h-7 w-7  bg-garden-option1 text-white'>{stock}</div>
			</Show>
		</div>
	)
}
