...
export const Item = ({ data }: IProps) => {
    ...
    return (
        ...
        <div className={`animate-area ${style.itemBanner}`} style={{ backgroundImage: ResolverPahtname(data.image) }}  >
            <div className={`mx-auto my-0 w-[90%] xl:w-[1280px] h-full flex justify-center items-center text-white relative z-10 ${data.align === 'l' ? 'md:justify-start' : 'md:justify-end'} md:px-6  `}>
                <div className='h-80 flex flex-col items-center md:items-start justify-center max-w-sm lg:max-w-[700px] '>
                    <h4 className='font-garden_bold text-base md:text-5xl md:font-extralight text-center md:text-left '>
                        {data.tittle || ''}
                    </h4>
