...
const FormTransferencia = ({ setShow, tipoPago, total }: Props) => {
	...
	return (
		<div className='relative pt-0 lg:p-10'>
			<button onClick={() => setShow('Transferencia')} className='absolute top-0 lg:left-20 p-1.5 text-white rounded-full bg-garden-option1'>
				<IconArrowLeft />
			</button>
			<h2 className='text-4xl font-bold text-center text-garden-option1'>{db.titulo}</h2>

			<h2 className='mt-10 text-xl font-bold text-center '>Realizar la transferencia con el siguiento numero de cuenta.</h2>
			...
			<Show
				...
				<div className='mx-auto my-5 w-60 h-60 '>
					<Image loading='lazy' src={db.imagenPrincipal?.url!} width={500} height={500} objectFit='contain' alt='imagen' />
				</div>
			</Show>
			<div className='relative flex justify-center '>
				<div className='flex items-center justify-between w-64 px-2.5 py-4 border border-gray-400 border-dashed '>
					<p className='text-sm font-semibold '>{db.numeroCuenta}</p>
					<button
						className='p-1 px-2 font-bold text-white bg-garden-option1'
						onClick={() => {
							copyTextToClipboard(db.numeroCuenta!)
							setCopy(true)
							setTimeout(() => {
								setCopy(false)
							}, 3000)
			...
			<div className='pt-10'>
				...
				<div className='flex justify-center pt-5'>
					<button
						onClick={() => setShow('TransferenciaIdUpload')}
						type='button'
