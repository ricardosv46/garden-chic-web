	...
	return (
		<>
			<div className='relative w-full h-[600px]  '>
				<Image src={`/imgs/servicios/${url.img!}`} objectPosition='top' layout='fill' objectFit='cover' />
				...
				<div className='mx-auto my-0 w-[90%] xl:w-[1280px] h-full flex  justify-center items-center text-white relative z-10'>
					<h2 className='text-center text-2xl md:text-4xl lg:text-6xl text-white font-garden_bold relative z-10 '>{url.title || ''}</h2>
				</div>
			</div>
			<Container className='flex flex-col-reverse lg:flex lg:flex-row py-10 gap-10 mt-10  '>
				<div className='flex flex-col gap-x-2 sm:gap-x-7 w-full lg:w-3/12 p-5 lg:px-3'>
					<p className='text-garden-option1 text-2xl md:text-3xl font-garden_medium pb-5'>Servicios</p>
					{serviciosLat.map((item, i) => (
						<div key={i} className='border-b-2 border-b-gray-200 py-4 lg:py-5'>
							<p
								...
								className='text-garden-option3 text-base lg:text-xl font-semibold hover:text-garden-option1 ease-in-out duration-300 cursor-pointer'>
								{item.text}
							</p>
						</div>
					))}

					<p className='text-garden-option1 text-2xl md:text-3xl font-garden_medium py-5'>Productos similares</p>
				...
				<div className='w-full lg:w-9/12 '>
					{url?.content.map((obj, i) => (
						<article className='' key={i}>
							<h2 className='text-garden-option1 font-garden_medium leading-none text-2xl md:text-3xl lg:text-5xl  '>
								{obj.tittle || ''}
							</h2>
							<div className='text-garden-option3 font-garden_regular text-base lg:text-lg font-light py-7 text-justify'>
								{obj?.subcontent?.map((obj2, j) => (
									<p className='pb-4' key={j}>
										{obj2 || ''}
									</p>
								))}
							</div>
					...
					<div className='mt-3'>
						<h2 className='text-garden-option1 font-garden_medium leading-none text-2xl md:text-3xl lg:text-5xl'>Proyectos</h2>
						<div className=' grid grid-cols-1 sm:grid-cols-2  gap-5  justify-items-center mt-10 '>
							{dataProyects.map((item, index) => {
								if (index < 5) {
									return <ProyectItem key={index} data={item} />
								}
							})}
							<div
								...
								<div className='aspect-square w-1/5 relative '>
									<Image layout='fill' src='/icons/png/iconProyect.png' alt='imagen proyecto' className='absolute ' />
								</div>
								<div className='flex flex-row text-sm lg:text-base  text-garden-option1 font-garden_regular '>
									<span>Ver más</span>
