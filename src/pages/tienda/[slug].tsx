...
interface Producto {
  producto: {
    galeria: Imagen[]
    CategoriaProducto: { titulo: string; slug: string }
    titulo: string
    precioOferta: number
...
const Productos = ({ producto }: Producto) => {
  ...
  const { db: productos, loading } = useCategoriaProductosSlug({
    estado: 'Activado',
    pagina: 1,
    numeroPagina: 5,
  ...
  useEffect(() => {
    if (precio?.max > 0) {
      const getData = async () => {
        await FunctionBusquedaAvanzada(handleData()).then((res) => setProductosFiltrados(res?.db!))
      }
      getData()
    }
  }, [precio?.max, precio.min])
          ...
          <div className='gap-5 lg:flex'>
            ...
            <div className='relative flex-1 '>
              ...
              <div className='flex items-center gap-10 mt-5'>
                ...
                {Product?.stockReal! <= 0 && <button className='px-4 py-1 text-lg font-semibold text-white bg-red-500'>Agotado</button>}
              </div>
            </div>
          </div>
          <div>
