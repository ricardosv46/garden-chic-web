		...
		<div>
			...
			<Container className='lg:flex py-10 gap-10'>
				<div className='w-full lg:w-9/12 '>
					<div className='lg:flex gap-5'>
						...
						<div className='flex-1 '>
							...
							<p className='text-gray-900 text-3xl  font-bold mt-3'>{Product?.titulo!}</p>
							<div className='w-5 h-0.5 bg-primary-300 my-5'></div>
							<p className='text-gray-900 text-5xl  '>S/ {Product?.precioOferta!}</p>

							<p className='text-gray-900 text-lg  my-5'>{Product?.descripcionLarga!}</p>
							<div className='flex justify-between border-2'>
								<div className='flex-1 flex justify-center items-center'>
									<input type='number' min='1' placeholder='1' className='w-16 p-2 focus:outline-none border-2 rounded-lg' />
								</div>

								<p className='flex-1 p-5 border-l-2 text-primary-300 text-center'>{Product?.titulo}</p>
					...
					<div>
						<p className='text-gray-900 text-3xl  font-bold mt-10'>Descripción</p>
						<p className='text-gray-900 text-lg  my-5'>{Product?.descripcionLarga}</p>
						<p className='text-gray-900 text-3xl  font-bold mt-10'>También te puede interesar</p>
						...
						<p className='text-gray-900 text-3xl  font-bold mt-10'>Productos relacionados</p>
						<div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5  justify-items-center py-10'>
							{productos.map((item, i) => {
								if (i > 2 && i < 5) {
									return (
				...
				<div className='w-full lg:w-3/12 px-3'>
					...
					<p className='text-gray-900 text-2xl font-bold py-10'>Productos similares</p>
					{productos.map((item, i) => {
						if (i < 5) {
							return (
